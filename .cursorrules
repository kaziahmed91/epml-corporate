...
- Always use strict equality (===) instead of loose equality (==) where applicable
...
- Utilize App Router for routing while utilizing route handlers (route.ts) for API routes in the App Router.
...
- Use useContext for shared state however, use sparingly to avoid unecessary re-renders.
- Implement proper state initialization. 
- For client side state or optimistic ui, use react-query library. 
...
7. For metadata (in .tsx files):
   ...
   ```
8. For error handling (in error.tsx):
   ```tsx
   'use client'
